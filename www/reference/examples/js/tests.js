// add entries here for new skins
var jxSkins = [
    {label: 'Delicious Theme', skin: 'delicious', image: 'images/delicious.png', active: false},
    {label: 'Crispin Theme', skin: 'crispin', image: 'images/crispin.png', active: false}
];

// read the user's last choice (if any) from a cookie
var defaultSkin = Cookie.read('jxtests.stylesheet') || 'Delicious Theme';

// make sure the right skin is active
jxSkins.some(function(skin){
    if (defaultSkin == skin.label) {
        skin.active = true;
        return true;
    }
});
...
jxSkins.each(function(skin, i) {
    ...
    new Asset.css('../../lib/jxskin_' + skin.skin + '.css', {
        rel: rel,
        title: skin.label
    });
});
...
new Asset.css('css/prettify.css', {rel:'stylesheet'});

// load the test css file
new Asset.css('css/tests.css', {rel:'stylesheet'});
...
window.addEvent('load', function() {
    ...
    // create a toolbar to hold our common actions
    new Element('div', {id: 'pageBar'}).
        inject($$('h1')[0], 'bottom');
            ...
            onChange: function() {
                ...
                $$('link').each(function(link) {
                    if (link.getAttribute('rel').indexOf("style") != -1 &&
                        ...
                        link.disabled = (link.getAttribute('title') != ss);
                    }
                });
                Cookie.write('jxtests.stylesheet', ss);
    ...
    new Asset.javascript('http://www.google-analytics.com/ga.js', {
        onload: function(){ 
            _gat._getTracker("UA-124914-7")._trackPageview();
        }
    });
